...
volatile uint8_t RFM69::SENDERID[ADDR_LEN];
volatile uint8_t RFM69::TARGETID[ADDR_LEN];     // should match _address
...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID[ADDR_LEN]) {
    const uint8_t CONFIG[][2] =
            {
                    ...
                    /* 0x07 */
                               {REG_FRFMSB,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFMSB_315 : (freqBand ==
                                                                                                         RF69_433MHZ
                                                                                                         ? RF_FRFMSB_433
                                                                                                         : (freqBand ==
                                                                                                            RF69_868MHZ
                                                                                                            ...
                                                                                                            : RF_FRFMSB_915)))},
                    /* 0x08 */
                               {REG_FRFMID,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFMID_315 : (freqBand ==
                                                                                                         RF69_433MHZ
                                                                                                         ? RF_FRFMID_433
                                                                                                         : (freqBand ==
                                                                                                            RF69_868MHZ
                                                                                                            ...
                                                                                                            : RF_FRFMID_915)))},
                    /* 0x09 */
                               {REG_FRFLSB,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFLSB_315 : (freqBand ==
                                                                                                         RF69_433MHZ
                                                                                                         ? RF_FRFLSB_433
                                                                                                         : (freqBand ==
                                                                                                            RF69_868MHZ
                    ...
                    /* 0x2F */
                               {REG_SYNCVALUE1,    0xDE},      // attempt to make this compatible with sync1 byte of RFM12B lib
                    /* 0x30 */
                               {REG_SYNCVALUE2,    0xAD}, // NETWORK ID
                    /* 0x37 */
    ...
    while (((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00) &&
           millis() - start < timeout); // wait for ModeReady
    ...
    selfPointer = this;
    memcpy((void *) _address, nodeID, ADDR_LEN);
    return true;
}
...
void RFM69::setAddress(uint8_t addr[ADDR_LEN]) {
    memcpy(_address, addr, 8);
...
void RFM69::send(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t bufferSize, bool requestACK) {
...
bool RFM69::sendWithRetry(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t bufferSize, uint8_t retries,
...
bool RFM69::ACKReceived(uint8_t fromNodeID[ADDR_LEN]) {
    if (receiveDone())
        return (memcmp((const void *) SENDERID, fromNodeID, ADDR_LEN) == 0 && ACK_RECEIVED);
    return false;
}

// check whether an ACK was requested in the last received packet (non-broadcasted packet)
bool RFM69::ACKRequested() {
    return (bool) ACK_REQUESTED;
}
...
void RFM69::sendACK(const void *buffer, uint8_t bufferSize) {
    ...
    memcpy((void *) sender, (const void *) SENDERID, ADDR_LEN);
    int16_t _RSSI = RSSI; // save payload received RSSI value
    writeReg(REG_PACKETCONFIG2, (readReg(REG_PACKETCONFIG2) & 0xFB) | RF_PACKET2_RXRESTART); // avoid RX deadlocks
    unsigned long now = millis();
    while (!canSend() && millis() - now < RF69_CSMA_LIMIT_MS) receiveDone();
    memcpy((void *) SENDERID, (const void *) sender, ADDR_LEN);
...
// internal function
void
RFM69::sendFrame(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t bufferSize, bool requestACK, bool sendACK) {
    ...
    SPI.transfer(bufferSize + (ADDR_LEN * 2) + 1);
    for (int i = 0; i < ADDR_LEN; i++) {
        SPI.transfer(toAddress[i]);
    }
    for (int i = 0; i < ADDR_LEN; i++) {
        SPI.transfer(_address[i]);
    }
...
void RFM69::interruptHandler() {
    ...
    if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY)) {
        ...
        for (int i = 0; i < ADDR_LEN; i++) {
            TARGETID[i] = SPI.transfer(0);
        }

        if (!(_promiscuousMode || memcmp((const void *) TARGETID, _address, ADDR_LEN) == 0) ||
            PAYLOADLEN < (ADDR_LEN * 2) + 1 ) {
            PAYLOADLEN = 0;
            unselect();
            receiveBegin();
            //digitalWrite(4, 0);
        ...
        DATALEN = PAYLOADLEN - (ADDR_LEN * 2) - 1;
        for (int i = 0; i < ADDR_LEN; i++) {
            SENDERID[i] = SPI.transfer(0);
        }
...
void RFM69::receiveBegin() {
    DATALEN = 0;
    memset((void *) SENDERID, 0, ADDR_LEN);
    memset((void *) TARGETID, 0, ADDR_LEN);
    PAYLOADLEN = 0;
    ACK_REQUESTED = 0;
    ACK_RECEIVED = 0;
    RSSI = 0;
...
// false = enable node/broadcast filtering to capture only frames sent to this address
