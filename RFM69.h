...
public:
    ...
    static volatile uint8_t SENDERID[ADDR_LEN];
    static volatile uint8_t TARGETID[ADDR_LEN]; // should match _address
    ...
    bool initialize(uint8_t freqBand, uint8_t ID[ADDR_LEN]);

    void setAddress(uint8_t addr[ADDR_LEN]);

    bool canSend();

    virtual void send(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t bufferSize, bool requestACK = false);

    virtual bool sendWithRetry(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t bufferSize, uint8_t retries = 2,
                               uint8_t retryWaitTime = 40); // 40ms roundtrip req for 61byte packets
    virtual bool receiveDone();

    bool ACKReceived(uint8_t fromNodeID[ADDR_LEN]);
...
protected:
    ...
    virtual void
    sendFrame(uint8_t toAddress[ADDR_LEN], const void *buffer, uint8_t size, bool requestACK = false,
              bool sendACK = false);
